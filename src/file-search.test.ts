...
describe('文件搜索和过滤工具', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.promises.writeFile(
      path.join(utilsDir, 'constants.js'),
      'module.exports = { API_URL: "http://api.test" };'
    );
    ...
    await fs.promises.writeFile(
      path.join(testDir, 'lib.test.js'),
      'describe("lib", () => { it("works", () => {}); });'
    );
  ...
  describe('基本文件搜索功能', () => {
    ...
    it('应该按文件内容搜索文件', async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(
        results.every(r => r.name.endsWith('.ts') || r.name.endsWith('.js'))
      ).toBe(true);
    });
    ...
    it('应该按正则表达式搜索文件名', async () => {
      ...
      const results = await searchEngine.searchByRegex(
        rootContext,
        /\.test\.(ts|js)$/
      );
    ...
    it('应该按文件大小范围搜索文件', async () => {
      ...
      const results = await searchEngine.searchBySize(rootContext, {
        min: 10,
        max: 1000,
      });
    ...
    it('应该按修改时间搜索文件', async () => {
      ...
      const results = await searchEngine.searchByModifiedTime(rootContext, {
        after: yesterday,
        before: tomorrow,
      });
  ...
  describe('高级搜索功能', () => {
    it('应该支持组合搜索条件', async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(
        results.every(r => r.name.includes('index') && r.name.endsWith('.ts'))
      ).toBe(true);
    });
    ...
    it('应该支持搜索结果分页', async () => {
      ...
      const results = await searchEngine.searchWithPagination(
        rootContext,
        '.*',
        {
          page: 1,
  ...
  describe('文件过滤功能', () => {
    it('应该按文件类型过滤', async () => {
      ...
      const results = await searchEngine.filterByType(
        rootContext,
        'javascript'
      );
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(
        results.every(r => r.name.endsWith('.js') || r.name.endsWith('.ts'))
      ).toBe(true);
    });
    ...
    it('应该支持排除模式过滤', async () => {
      ...
      expect(
        results.every(
          r => !r.name.includes('.test.') && !r.name.endsWith('.md')
        )
      ).toBe(true);
    ...
    it('应该支持包含模式过滤', async () => {
      ...
      expect(
        results.every(r => r.path.includes('src') || r.path.includes('tests'))
      ).toBe(true);
    });
    ...
    it('应该支持自定义过滤函数', async () => {
      ...
      const results = await searchEngine.filterWithCustomFunction(
        rootContext,
        file => {
      ...
      expect(Array.isArray(results)).toBe(true);
      expect(
        results.every(r => r.name.length > 5 && r.name.includes('.'))
      ).toBe(true);
    });
  });
  ...
  describe('搜索索引和缓存功能', () => {
    ...
    it('应该支持搜索结果缓存', async () => {
      ...
      const results1 = await searchEngine.searchByContent(
        rootContext,
        'export'
      );
      ...
      const results2 = await searchEngine.searchByContent(
        rootContext,
        'export'
      );
  ...
  describe('搜索结果处理功能', () => {
    it('应该高亮搜索结果', async () => {
      ...
      const results = await searchEngine.searchWithHighlight(
        rootContext,
        'export',
        {
          highlightTag: '<mark>',
