...
describe('工具管理器', () => {
  ...
  describe('工具查询', () => {
    ...
    it('应该能够按名称查找工具', () => {
      const mockTools = [
        { name: 'calculator', description: '计算器', schema: {}, handler: jest.fn() },
        { name: 'file-reader', description: '文件读取器', schema: {}, handler: jest.fn() }
      ];
    ...
    it('应该能够按类别筛选工具', () => {
      const mockTools = [
        { name: 'add', description: '加法', category: 'math', schema: {}, handler: jest.fn() },
        { name: 'read-file', description: '读文件', category: 'file', schema: {}, handler: jest.fn() },
        { name: 'multiply', description: '乘法', category: 'math', schema: {}, handler: jest.fn() }
      ];
  ...
  describe('工具执行', () => {
    it('应该能够执行工具', async () => {
      ...
      expect(mockMCPServer.executeTool).toHaveBeenCalledWith('add', { a: 20, b: 22 });
    });

    it('应该能够处理工具执行错误', async () => {
      mockMCPServer.executeTool.mockRejectedValue(new Error('工具执行失败'));

      await expect(toolManager.executeTool('invalid-tool', {}))
  ...
  describe('工作空间集成', () => {
    it('应该能够创建工具工作空间', () => {
      ...
      expect(mockWorkspaceManager.createWorkspace).toHaveBeenCalledWith(workspace);
      expect(mockMCPServer.setWorkspaceManager).toHaveBeenCalledWith(mockWorkspaceManager);
    });
    ...
    it('应该能够向工作空间添加工具', () => {
      ...
      expect(mockWorkspaceManager.addToolToWorkspace).toHaveBeenCalledWith('math-workspace', tool);
    });

    it('应该能够切换工作空间', () => {
      toolManager.switchWorkspace('math-workspace');
      
      expect(mockMCPServer.setCurrentWorkspace).toHaveBeenCalledWith('math-workspace');
    });
