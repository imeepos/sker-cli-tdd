...
export class FileOperationsManager {
   ...
   */
  async readFile(context: FileContext, encoding: BufferEncoding = 'utf8'): Promise<string> {
    try {
      const content = await fs.promises.readFile(context.path, encoding);
      this.addHistoryEntry(context.path, 'read', content.length);
      return content;
    } catch (error) {
      throw new Error(`无法读取文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   */
  async readLines(context: FileContext, startLine: number, endLine: number, encoding: BufferEncoding = 'utf8'): Promise<string[]> {
   ...
   * @returns Promise，解析为行数组
   */
  async readAllLines(context: FileContext, encoding: BufferEncoding = 'utf8'): Promise<string[]> {
    const content = await this.readFile(context, encoding);
  ...
  async readBinary(context: FileContext): Promise<Buffer> {
    try {
      ...
      throw new Error(`无法读取二进制文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   * @param encoding 编码格式，默认为utf8
   */
  async writeFile(context: FileContext, content: string, encoding: BufferEncoding = 'utf8'): Promise<void> {
    try {
      await fs.promises.writeFile(context.path, content, encoding);
      this.addHistoryEntry(context.path, 'write', content.length);
    } catch (error) {
      throw new Error(`无法写入文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   * @param encoding 编码格式，默认为utf8
   */
  async appendFile(context: FileContext, content: string, encoding: BufferEncoding = 'utf8'): Promise<void> {
    try {
      await fs.promises.appendFile(context.path, content, encoding);
      this.addHistoryEntry(context.path, 'modify', content.length);
    } catch (error) {
      throw new Error(`无法追加文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   * @param encoding 编码格式，默认为utf8
   */
  async writeLines(context: FileContext, lines: string[], encoding: BufferEncoding = 'utf8'): Promise<void> {
  ...
  async writeBinary(filePath: string, data: Buffer): Promise<void> {
    try {
      ...
      throw new Error(`无法写入二进制文件 ${filePath}: ${(error as Error).message}`);
    }
  }
   ...
   */
  async createFile(filePath: string, content: string = '', encoding: BufferEncoding = 'utf8'): Promise<FileContext> {
    try {
   ...
   */
  async copyFile(sourceContext: FileContext, destPath: string): Promise<FileContext> {
    try {
      ...
      throw new Error(`无法复制文件 ${sourceContext.path} 到 ${destPath}: ${(error as Error).message}`);
    }
  }
   ...
   * @returns Promise，解析为目标FileContext
   */
  async moveFile(sourceContext: FileContext, destPath: string): Promise<FileContext> {
    try {
      ...
      throw new Error(`无法移动文件 ${sourceContext.path} 到 ${destPath}: ${(error as Error).message}`);
    }
  }
  ...
  async deleteFile(context: FileContext): Promise<void> {
    try {
      ...
      throw new Error(`无法删除文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   */
  async renameFile(context: FileContext, newName: string): Promise<FileContext> {
   ...
   */
  async watchFile(context: FileContext, callback: (event: string, filename?: string) => void): Promise<FileWatcher> {
    try {
      ...
      throw new Error(`无法监控文件 ${context.path}: ${(error as Error).message}`);
    }
  }
   ...
   */
  async readMultipleFiles(contexts: FileContext[], encoding: BufferEncoding = 'utf8'): Promise<FileReadResult[]> {
    ...
    for (const context of contexts) {
      try {
        ...
        console.warn(`无法读取文件 ${context.path}: ${(error as Error).message}`);
      }
    }
   ...
   */
  async writeMultipleFiles(operations: BatchWriteOperation[]): Promise<FileContext[]> {
    ...
    for (const operation of operations) {
      try {
        ...
        console.warn(`无法写入文件 ${operation.path}: ${(error as Error).message}`);
      }
    }
   ...
   * @private
   */
  private addHistoryEntry(filePath: string, operation: FileHistoryEntry['operation'], size: number): void {
