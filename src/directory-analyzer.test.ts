...
describe('目录结构分析工具', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.promises.writeFile(path.join(testDir, 'index.test.ts'), 'test("should work", () => {});');
  ...
  describe('目录结构分析功能', () => {
    ...
    it('应该分析目录大小分布', async () => {
      ...
      const sizeDistribution = await analyzer.analyzeSizeDistribution(rootContext);
  ...
  describe('目录结构比较功能', () => {
    it('应该比较两个目录结构的差异', async () => {
      // 创建第二个测试目录
      const tempDir2 = path.join(os.tmpdir(), 'sker-dir-analyzer-test2-' + Date.now());
      ...
      try {
        ...
        await fs.promises.writeFile(path.join(tempDir2, 'package.json'), '{"name": "test2"}');
        await fs.promises.writeFile(path.join(srcDir2, 'index.ts'), 'export * from "./lib";');
        await fs.promises.writeFile(path.join(srcDir2, 'utils.ts'), 'export const utils = true;'); // 新文件
        ...
        const comparison = await analyzer.compareStructures(rootContext1, rootContext2);
    ...
    it('应该生成结构变化报告', async () => {
      ...
      const changeReport = await analyzer.generateChangeReport(rootContext, updatedContext);
