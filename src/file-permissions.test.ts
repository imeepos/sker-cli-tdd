...
describe('文件权限管理工具', () => {
  ...
  describe('权限修改功能', () => {
    ...
    it('应该使用八进制模式设置权限', async () => {
      ...
      await expect(
        permissionsManager.setMode(fileContext, 0o755)
  ...
  describe('批量权限操作', () => {
    it('应该批量检查文件夹中所有文件的权限', async () => {
      ...
      const folderPermissions =
        await permissionsManager.checkFolderPermissions(rootContext);
  ...
  describe('权限分析功能', () => {
    it('应该生成权限报告', async () => {
      ...
      const report =
        await permissionsManager.generatePermissionsReport(rootContext);
